...
const SettingsPage = () => {
  ...
  const fetchAuditLogs = async () => {
    ...
    try {
      ...
      if (auditFilters.record_type) {
        params.append('record_type', auditFilters.record_type);
      }
  ...
  const handleImportCategories = async (event) => {
    ...
    if (!['xlsx', 'xls', 'csv'].includes(fileExtension)) {
      toast.error('Por favor selecciona un archivo .xlsx o .csv');
      return;
    }
    ...
    try {
      ...
      reader.onload = (e) => {
        try {
          ...
          // Extraer categorías (primera columna, saltando encabezado si existe)
          ...
          jsonData.forEach((row, index) => {
            if (row && row[0]) {
              ...
              if (category && 
                  ...
                  category.toLowerCase() !== 'categoría' &&
                  category.toLowerCase() !== 'category' &&
                  !importedCategories.includes(category) &&
          ...
          toast.success(`${importedCategories.length} categoría${importedCategories.length > 1 ? 's' : ''} importada${importedCategories.length > 1 ? 's' : ''} exitosamente`);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-8">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Audit Logs Tab - Solo para account_admin */}
        {activeTab === 'audit' && user?.role === 'account_admin' && (
          <div 
            className="bg-white border-2 border-slate-900 rounded-xl p-8"
            style={{ boxShadow: '8px 8px 0px 0px rgba(15,23,42,1)' }}
          >
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-2xl font-bold text-slate-900">Registros de Auditoría</h2>
              </div>
              <button
                onClick={fetchAuditLogs}
                disabled={auditLoading}
                className="flex items-center gap-2 px-4 py-2 bg-blue-100 border-2 border-slate-900 rounded-lg font-bold hover:bg-blue-200 transition-all disabled:opacity-50"
                style={{ boxShadow: '2px 2px 0px 0px rgba(15,23,42,1)' }}
              >
            ...
            <div className="bg-blue-50 border-2 border-blue-900 rounded-lg p-4 mb-6">
              <p className="text-sm text-blue-900">
                <strong>📋 Historial de cambios en registros históricos</strong><br/>
                Visualiza quién editó o eliminó ventas, egresos o ingresos, y cuándo.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <div>
                ...
                <select
                  value={auditFilters.record_type}
                  onChange={(e) => setAuditFilters({ ...auditFilters, record_type: e.target.value })}
            ...
            {auditLoading ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b-2 border-slate-900">
                      <th className="text-left py-3 px-4 font-bold text-slate-900">Fecha/Hora</th>
                      <th className="text-left py-3 px-4 font-bold text-slate-900">Usuario</th>
                      <th className="text-left py-3 px-4 font-bold text-slate-900">Acción</th>
                      <th className="text-left py-3 px-4 font-bold text-slate-900">Tipo</th>
                      <th className="text-left py-3 px-4 font-bold text-slate-900">Detalles</th>
                    </tr>
                  </thead>
                  <tbody>
                    {auditLogs.map((log) => (
                      <tr key={log.id} className="border-b border-slate-200 hover:bg-slate-50">
                        ...
                        <td className="py-3 px-4">
                          <span className={`px-3 py-1 rounded-lg text-xs font-bold ${
                            ...
                            {log.action === 'delete' ? '🗑️ Eliminado' : '✏️ Editado'}
                          </span>
                        </td>
                        <td className="py-3 px-4">
                          <span className="px-2 py-1 bg-slate-100 rounded text-xs font-medium">
                            {log.record_type === 'sale' && 'Venta'}
                            {log.record_type === 'expense' && 'Egreso'}
                            {log.record_type === 'income' && 'Ingreso'}
                          </span>
                        </td>
                        <td className="py-3 px-4">
                          <details className="cursor-pointer">
                            <summary className="text-sm text-blue-600 hover:underline font-medium">
                              Ver cambios
                            </summary>
                            <div className="mt-2 p-3 bg-slate-50 rounded-lg text-xs space-y-2">
                              {log.action === 'update' && log.old_data && log.new_data && (
                                <>
                                  <div>
                                    ...
                                    <pre className="text-slate-700 mt-1 whitespace-pre-wrap">
                              ...
                              {log.action === 'delete' && log.old_data && (
                                <div>
                                  <strong className="text-red-700">Registro eliminado:</strong>
                                  <pre className="text-slate-700 mt-1 whitespace-pre-wrap">
                                    {JSON.stringify(log.old_data, null, 2)}
                                  </pre>
                                </div>
