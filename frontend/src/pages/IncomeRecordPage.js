...
import { ChevronLeft, ChevronRight, Home, Calendar as CalendarIcon, TrendingUp, DollarSign, Plus, Trash2, Edit2 } from 'lucide-react';
...
const IncomeRecordPage = () => {
  ...
  const [showPastIncomeForm, setShowPastIncomeForm] = useState(false);
  ...
  const fetchCalendarData = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${API}/income-records/calendar/${currentYear}/${currentMonth}`);
  ...
  const fetchDayIncome = async (day) => {
    const dateStr = `${currentYear}-${currentMonth.toString().padStart(2, '0')}-${day.toString().padStart(2, '0')}`;
    try {
      const response = await axios.get(`${API}/income-records/day/${dateStr}`);
      setDayIncome(response.data.incomes);
      setSelectedDay(day);
    } catch (error) {
      console.error('Error al cargar ingresos del día:', error);
      toast.error('Error al cargar ingresos del día');
    }
  };
  ...
  const handleEditIncome = (income) => {
    setEditModal({ open: true, data: { ...income } });
  };
  ...
  const handleUpdateIncome = async (e) => {
    ...
    try {
      ...
      await axios.put(`${API}/other-income/${editModal.data.id}`, payload);
    ...
    <div className="min-h-screen" style={{ backgroundColor: '#F4F4F0' }}>
      ...
      <header className="bg-white border-b-2 border-slate-900 py-4 px-6 md:px-8 sticky top-0 z-50">
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-3">
            ...
            {canEdit && (
              <button
                onClick={() => setShowPastIncomeForm(true)}
      ...
      <div className="p-6 md:p-8 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Panel Derecho: Resumen y Ingresos del Día */}
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-slate-100 border-2 border-slate-900 rounded-lg">
                  <p className="text-xs font-bold uppercase text-slate-600 mb-1">Ingresos del Mes</p>
                  <p className="text-xl font-bold text-slate-900">
                    {calendarData?.total_income_count || 0} registros
                  </p>
                </div>
              </div>
            </div>
      ...
      {/* Modal de Registrar Ingreso Pasado */}
      {showPastIncomeForm && (
        <PastIncomeForm
          onClose={() => setShowPastIncomeForm(false)}
          onSuccess={handlePastIncomeSuccess}
      ...
      {editModal.open && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          ...
          >
            <h3 className="text-xl font-bold mb-4">Editar Otro Ingreso</h3>
...
export default IncomeRecordPage;
